...
{
    var corsOriginOptions = builder.Configuration.GetSection(nameof(CorsOriginOptions)).GetChildren().ToArray();

    options.AddPolicy("default", pol => pol.WithOrigins(corsOriginOptions[0].Value!));
});
builder.Services.AddSwaggerGen(options =>
{
    var openApiOptions = builder.Configuration.GetSection(nameof(OpenApiOptions)).GetChildren().ToArray();
